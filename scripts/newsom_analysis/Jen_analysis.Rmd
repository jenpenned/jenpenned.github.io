...
# This reads in tweet dataframes, formats it to include governor handle information, and trims the dates the tweets were created to the timeframe we are interested in (2019-03-01 to 2019-12-31)

## read in all the tweets, including the latest batch of updates
all = Sys.glob('data/everything-*.csv') %>% 
  sort(decreasing = TRUE) %>% 
  `[`(1)

everything <- read_csv(all, guess_max = 50000)
...
newsomTweets = everything %>% 
  filter(screen_name == "GavinNewsom" | screen_name == "CAgovernor") %>%
  filter(created_at >= make_date(2017, 1, 1))
```

#### Which @GavinNewsom tweets received the most likes in the past 3 years?
...
most_reactions = NewsomFB %>% 
  select(date, text, reactions_cleaned, shares_cleaned, comments_cleaned) %>% 
...
most_shares = NewsomFB %>% 
  select(date,text, shares_cleaned, reactions_cleaned, comments_cleaned) %>% 
...
Gavin Newsom's personal account ranks 14th in number of tweets compared to other governors' personal or official Twitter acounts. He is outpaced by his official account, @CAgovenor, which has tweeted 1.7 times as much in this same time period.
 
```{r governors_distinct, echo=FALSE}
total_tweets = formatted_everything %>% 
  group_by(name, screen_name) %>%
  summarize(tweets = n()) %>%
  arrange(desc(tweets))
datatable(total_tweets)
```

And if we looked at which governor tweeted the most overall (from their official and personal accounts), Newsom becomes the second most prolific governor on Twitter in this time period. 
...
If we looked at only governors' personal Twitter accounts, Gavin Newsom ranks third in Trump-related tweets.
...
top10trumpaccounts4 = percent_trump_personal %>% 
  ungroup() %>% 
  select(screen_name, trump_tweets) %>% 
  head(10)

#top10trumpaccounts4 %>%arrange(desc(trump_tweets))

trumptweetstop10order4 = formatted_everything %>%
  ...
  filter(mentions_trump, !mentions_family) %>% 
  filter(screen_name %in% top10trumpaccounts4$screen_name) %>% 
  select(screen_name, created_at) %>% 
  group_by(screen_name) %>% 
  mutate(tweetorder = rank(created_at))

trumptweetstop10order4 %>% 
  ggplot(aes(created_at, tweetorder, color = screen_name)) +
  geom_line() +
  theme_minimal()+
  # Legend title
  scale_color_discrete(name = "Twitter handles") +
  labs(title = "Tweeting patterns of the top 10 governors' personal accounts that mention Trump",
...
top10trumpaccounts = percent_trump %>% 
  ungroup() %>% 
  select(screen_name, trump_tweets) %>% 
  head(10)

#top10trumpaccounts %>% arrange(desc(trump_tweets))

trumptweetstop10order = formatted_everything %>%
  select(screen_name, created_at, text) %>% 
  mutate(
    mentions_trump = str_detect(str_to_lower(text), 'trump'),
    mentions_family = str_detect(text, paste(c("IvankaTrump", "Ivanka Trump", "EricTrump", "DonaldJTrumpJr"), collapse = "|"))
  ...
  filter(mentions_trump, !mentions_family) %>% 
  filter(screen_name %in% top10trumpaccounts$screen_name) %>% 
  select(screen_name, created_at) %>% 
  group_by(screen_name) %>% 
  mutate(tweetorder = rank(created_at))

#trumptweetstop10order
...
trumptweetstop10order %>% 
  ...
  # Legend title
  scale_color_discrete(name = "Twitter handles") +
  labs(title = "Tweeting patterns of the top 10 governors' accounts that mention Trump",
...
top10trumpaccounts2 = percent_trump_combined %>% 
  ungroup() %>% 
...
trumptweetstop10order2 = formatted_everything %>%
  ...
  filter(mentions_trump, !mentions_family) %>% 
  filter(state %in% top10trumpaccounts2$state) %>% 
...
trumptweetstop10order2 %>% 
  ...
  # Legend title
  scale_color_discrete(name = "Twitter handles") +
  labs(title = "Tweeting patterns of the top 10 governors that mention Trump",
